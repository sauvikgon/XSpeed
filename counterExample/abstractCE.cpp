 ...
 * are passed as an argument to this function.
 */
nlopt::opt abstractCE::build_nlp(double tolerance,
		std::vector<double> fixedStartPoint[]) {
	...
	std::cout << "nlopt problem dimension (number of Dwell Time) = " << optD
			<< std::endl;
//	nlopt::opt myopt(nlopt::LN_AUGLAG, optD); // derivative free
//	nlopt::opt myopt(nlopt::LN_COBYLA, optD); // derivative free
...
/**
 * Implementing a routine for building nlp from fixed start point of trajectory segments
 */
/*nlopt::opt abstractCE::build_nlp(std::vector<double> x0[]){
...
concreteCE::ptr abstractCE::gen_concreteCE_iterative(double tolerance,
	...
	//std::vector<std::vector<double>> x0(N, std::vector<double>(dim)); // to contain the fixed start-points.
	...
	try {
		while (minf > tolerance) {
			...
			//todo:from this vector, get the N vectors in x0 vector: Amit may help
			if (res < tolerance)
				break;

			// solve the nlp with fixed start points.
			myoptDwellTime = build_nlp(tolerance, x0);
			myoptDwellTime.optimize(dwell_times, minf);
		}
	} catch (std::exception &e) {
		std::cout << e.what() << std::endl;
	}
	...
	if (minf > tolerance) {
		...
		// Todo: construct a concrete ce from the opt. result., Rajarshi to code
	}
