...
void setOscillatorTimed(hybrid_automata& Hybrid_Automata,
	...
	t = transition::ptr(
			new transition(2, "hop2", 2, 4, guard_polytope, assignment));

	Out_Going_Trans_fromloc3.push_back(t);
	l = location::ptr(
			new location(2, "loc3", system_dynamics, invariant, true,
	...
	t = transition::ptr(
			new transition(3, "hop4", 3, 1, guard_polytope, assignment));

	Out_Going_Trans_frompp.push_back(t);
	l = location::ptr(
			new location(3, "pp", system_dynamics, invariant, true,
					Out_Going_Trans_frompp));
	Hybrid_Automata.addLocation(l);
	...
	t = transition::ptr(
			new transition(4, "hop3", 4, 3, guard_polytope, assignment));
