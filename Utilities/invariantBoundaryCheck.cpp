...
unsigned int InvariantBoundaryCheck(Dynamics& SystemDynamics,
		supportFunctionProvider::ptr Initial, ReachabilityParameters& ReachParameters, polytope::ptr invariant, int lp_solver_type_choosen) {
...
//#pragma omp parallel for
	for (int eachInvariantDirection = 0; eachInvariantDirection < numberOfInvariants; eachInvariantDirection++) {
		...
		for (int i = 0; i < dimension; i++) {
			rVariable[i] = invariant->getCoeffMatrix()(eachInvariantDirection, i); //First vector positive
...
// ************************************************ For Positive Direction Starts ************************************************
		...
		if (!Initial->getIsEmpty()){ //set glpk constraints If not an empty polytope
			s_per_thread_I.setConstraints(ReachParameters.X0->getCoeffMatrix(),ReachParameters.X0->getColumnVector(),ReachParameters.X0->getInEqualitySign());
