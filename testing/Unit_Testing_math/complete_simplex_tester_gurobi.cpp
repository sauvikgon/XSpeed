...
struct Example_Gurobi_LPSolver {
 ...
 lp.setConstraints(ConstraintsMatrixI, boundValueI, boundSignI);
 status = lp.Compute_LPP(direction);
 cout << "\n\n\nMinimum in Left direction(give right value)= " << status
 << endl;
 ...
 cout << "\n\n\nMinimum in Down direction  AMIT SHOULD BE -2  IS IT? = "
 << test << endl;
...
//lp.setMin_Or_Max(1);	//1 for GLP_MIN
 ...
 std::cout
 << "\nGurobi TESTING USING Sequential CALLS :: Total Number of ITERATIONS = "
