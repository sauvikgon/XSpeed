/*
 * complete_simplex_tester_for_Gurobi.cpp
 *
...
SUITE(gurobi_lp_solver_TestSuiteComplete) {
...
//	cout << "\n\n\nMaximum in Down direction= " << status << endl;
	...
	lp.setConstraints(ConstraintsMatrixI, boundValueI, boundSignI);
	double test = lp.Compute_LPP(direction);
...
//	status = lp1.Compute_LPP(direction);
//lp.setMin_Or_Max(1);	//1 for GLP_MIN
...
TEST_FIXTURE(Example_Gurobi_LPSolver, lp_solver_1stQuadrant_Test2) {
...
TEST_FIXTURE(Example_Gurobi_LPSolver, lp_solver_Testing3) {
	//Test region on the 1stQuadrant
	/*
	 row = 4;
	 col = 2;
	 ...
	 direction[0] = 0;
	 direction[1] = -1;
	 //	std::cout << "Calling Sequential Testing for Gurobi" << endl;
	 //	std::cout << "**************************************" << endl;
	 ...
	 std::cout << "\nBoost Time taken:Wall  (in Seconds) = "<< wall_clock / (double) 1000 << std::endl;
	 std::cout << "\nBoost Time taken:User  (in Seconds) = "<< user_clock / (double) 1000 << std::endl;
	 std::cout << "\nBoost Time taken:System  (in Seconds) = "<< system_clock / (double) 1000 << std::endl;
	 std::cout<< "\nGurobi TESTING USING Sequential CALLS :: Total Number of ITERATIONS = "<< number_of_times << endl;
	 */

}

/*
 TEST_FIXTURE(Example_Gurobi_LPSolver, lp_solver_Test_OMP) {
 //Test region on the 1stQuadrant
 row = 4;
 col = 2;
...
TEST_FIXTURE(Example_Gurobi_LPSolver, lp_solver_Testing4) {
	//Test region on the 1stQuadrant
	row = 4;
	col = 3;
	...
	lp.setMin_Or_Max(2);	//2 for GLP_MAX
	lp.setConstraints(ConstraintsMatrixI, boundValueI, boundSignI);
	double result = lp.Compute_LPP(direction);
	//std::cout << "\nResult of the Problem using Gurobi = " << result << endl;
	out << "";
	out << result;
	proper << "";
	proper << "10.2";
...
/*
 ...
 TEST_FIXTURE(Example_Gurobi_LPSolver, lp_solver_Test_Sequential_ToCompareWithGurobi) {
 //Test region on the 1stQuadrant
...
TEST_FIXTURE(Example_Gurobi_LPSolver, lp_solver_Testing5) {
