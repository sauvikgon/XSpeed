...
TEST_FIXTURE(Example, TestManySimulationCircle) {
	hybrid_automata ha;
	std::list<initial_state::ptr> init_state;
	ReachabilityParameters reach_parameters;
...
TEST_FIXTURE(Example, TestHaSimulationCircle) {
	hybrid_automata ha;
	std::list<initial_state::ptr> init_state;
	ReachabilityParameters reach_parameters;
	...
	unsigned int max_jumps = 10;
	for(unsigned int i=0;i<n;i++)
	{
		sim->simulateHa(start_pts[i],0,reach_parameters.TimeBound,ha, max_jumps);
//		sim->print_trace_to_outfile("rotation_trace");
	}

}

TEST_FIXTURE(Example, TestParHaSimulationCircle) {
	hybrid_automata ha;
	std::list<initial_state::ptr> init_state;
	ReachabilityParameters reach_parameters;
