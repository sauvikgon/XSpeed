...
private:
	...
	std::list<transition> Out_Going_Transitions;
public:
	location();
	location(int Loc_ID, string Name, Dynamics System_Dynamics, polytope::ptr Invariant,  bool inv_exists, std::list<transition> Out_Going_Trans);
	...
	void add_Out_Going_Transitions(transition& t);
	std::list<transition>& getOut_Going_Transitions();
	int getLocId() const;
