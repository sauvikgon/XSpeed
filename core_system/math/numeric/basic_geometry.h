...
template<class scalar_type>
tribool is_ccw(const scalar_type& ax, const scalar_type& ay,
		const scalar_type& bx, const scalar_type& by, const scalar_type& cx,
		const scalar_type& cy) {
	scalar_type xmax = std::max(std::max(std::abs(ax), std::abs(bx)),
			std::abs(cx));
	scalar_type ymax = std::max(std::max(std::abs(ay), std::abs(by)),
			std::abs(cy));
