...
std::ostream& operator<<(std::ostream& os,
		const boost::numeric::interval<scalar_type, policy>& p) {
	os << "[" << p.lower() << "," << p.upper() << "]";
	return os;
}
...
public:
	static inline boost::numeric::interval<T, Policies> convert(
			const double& x) {
		return boost::numeric::interval<T, Policies>(x);
	}
};

template<class T, class Policies> inline boost::numeric::interval<T, Policies> pow(
