...
public:
	...
	interval(scalar_type l, scalar_type u) :
			my_lower(scalar_with_infinity<scalar_type>(l)), my_upper(
					scalar_with_infinity<scalar_type>(u)) {
	}
	~interval() {
	}
	...
	scalar_type size() const {
		...
		if (is_empty())
			throw std::runtime_error(
					"interval: requested size of empty interval");
...
std::list<interval<scalar_type> > intersect(
		const std::list<interval<scalar_type> >& L1,
		const std::list<interval<scalar_type> >& L2) {
	typedef std::list<interval<scalar_type> > intv_list;
	intv_list L;

	for (typename intv_list::const_iterator i1 = L1.begin(); i1 != L1.end();
			++i1) {
		for (typename intv_list::const_iterator i2 = L2.begin(); i2 != L2.end();
				++i2) {
...
std::ostream& operator<<(std::ostream& os,
		const math::numeric::interval<scalar_type>& intv) {
